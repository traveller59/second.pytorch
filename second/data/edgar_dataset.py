...
class EdgarDataset(Dataset):
    ...
    def __init__(self,
        ...
        self._edgar_infos = infos

        print("File: ", info_path, "\nremain number of infos:",
                len(self._edgar_infos), "\n")
        self._class_names = class_names
        self._prep_func = prep_func

    def __len__(self):
        return len(self._edgar_infos)

    def convert_detection_to_edgar_annos(self, detection):
        class_names = self._class_names
        det_image_idxes = [det["metadata"]["image_idx"] for det in detection]
        gt_image_idxes = [
            info["image"]["image_idx"] for info in self._edgar_infos
        ]
        annos = []
        for i in range(len(detection)):
            det_idx = det_image_idxes[i]
            det = detection[i]
            # info = self._edgar_infos[gt_image_idxes.index(det_idx)]
            info = self._edgar_infos[i]
            calib = info["calib"]
            rect = calib["R0_rect"]
            ...
            anno = edgar.get_start_result_anno()
            num_example = 0
            ...
            for j in range(box3d_lidar.shape[0]):
                ...
                # convert center format to edgar format
            ...
            if num_example != 0:
                ...
                annos.append(edgar.empty_result_anno())
    ...
    def evaluation(self, detections, output_dir):
        ...
        If you want to eval by my EDGAR eval function, you must 
        ...
        zero.
        """
        if "annos" not in self._edgar_infos[0]:
            return None
        gt_annos = [info["annos"] for info in self._edgar_infos]
        dt_annos = self.convert_detection_to_edgar_annos(detections)
        # firstly convert standard detection to edgar-format dt annos
        z_axis = 1  # EDGAR camera format use y as regular "z" axis.
        z_center = 1.0  # EDGAR camera box's center is [0.5, 1, 0.5]
    ...
    def get_sensor_data(self, query):
        ...
        if isinstance(query, dict):
            ...
            idx = query["lidar"]["idx"]
        info = self._edgar_infos[idx]
        # Added printing step info by Jim
        print('EDGAR data keys info for index (#', idx,') : ', info.keys())
        res = {
            "lidar": {
                "type": "lidar",
                "points": None,
        ...
        if 'annos' in info:
            ...
            annos = edgar.remove_dontcare(annos)
            locs = annos["location"]
            ...
            # edgar [0.5, 0.5, 0] center to [0.5, 0.5, 0.5]
...
def convert_to_edgar_info_version2(info):
    """convert edgar info v1 to v2 if possible.
    """
...
def edgar_anno_to_label_file(annos, folder):
    folder = Path(folder)
    for anno in annos:
        ...
        for j in range(anno["bbox"].shape[0]):
            ...
            label_line = edgar.edgar_result_line(label_dict)
            label_lines.append(label_line)
        label_file = folder / f"{edgar.get_image_index_str(image_idx)}.txt"
        label_str = '\n'.join(label_lines)
...
def _calculate_num_points_in_gt(data_path,
    ...
    for info in infos:
        ...
        # annos = edgar.filter_edgar_anno(annos, ['DontCare'])
...
def create_edgar_info_file(data_path, save_path=None, relative_path=True):
    ...
    edgar_infos_train = edgar.get_edgar_image_info(
        data_path,
        training=True,
        velodyne=True,
        calib=True,
    ...
    _calculate_num_points_in_gt(data_path, edgar_infos_train, relative_path)
    filename = save_path / 'edgar_infos_train.pkl'
    print(f"Edgar info train file is saved to {filename}")
    with open(filename, 'wb') as f:
        pickle.dump(edgar_infos_train, f)
    edgar_infos_val = edgar.get_edgar_image_info(
        data_path,
        training=True,
        velodyne=True,
        calib=True,
        ...
        relative_path=relative_path)
    _calculate_num_points_in_gt(data_path, edgar_infos_val, relative_path)
    filename = save_path / 'edgar_infos_val.pkl'
    print(f"Edgar info val file is saved to {filename}")
    with open(filename, 'wb') as f:
        pickle.dump(edgar_infos_val, f)
    filename = save_path / 'edgar_infos_trainval.pkl'
    print(f"Edgar info trainval file is saved to {filename}")
    with open(filename, 'wb') as f:
        pickle.dump(edgar_infos_train + edgar_infos_val, f)

    edgar_infos_test = edgar.get_edgar_image_info(
        data_path,
        training=False,
        label_info=False,
        velodyne=True,
        ...
        relative_path=relative_path)
    filename = save_path / 'edgar_infos_test.pkl'
    print(f"Edgar info test file is saved to {filename}")
    print(f"Edgar info test file info count {len(edgar_infos_test)}")
    with open(filename, 'wb') as f:
        pickle.dump(edgar_infos_test, f)
...
def _create_reduced_point_cloud(data_path,
                                ...
                                back=False):
    with open(info_path, 'rb') as f:
        edgar_infos = pickle.load(f)
    for info in prog_bar(edgar_infos):
        pc_info = info["point_cloud"]
        image_info = info["image"]
        calib = info["calib"]
...
def create_reduced_point_cloud(data_path,
    ...
    if train_info_path is None:
        train_info_path = Path(data_path) / 'edgar_infos_train.pkl'
    if val_info_path is None:
        val_info_path = Path(data_path) / 'edgar_infos_val.pkl'
    if test_info_path is None:
        test_info_path = Path(data_path) / 'edgar_infos_test.pkl'
